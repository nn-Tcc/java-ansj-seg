package com.tr.alg.library;

import com.tr.alg.util.MyStaticValue;
...
import static com.tr.alg.library.NgramLibrary.LOG;
        ...
        if (lib.length > 0) {
            ...
            for (File file : lib) {
                try (BufferedReader br = IOUtil.getReader(file, "utf-8")) {
                    ...
                    LOG.warn("Init ambiguity library error :{}, path: {}" + e.getMessage() + file.getPath());
                }
            }

            LOG.info("Init ambiguity library ok!");

        } else {
            LOG.warn("Init ambiguity library warning :{} because : file not found or failed to read !");
        }
        ...
        if (lib.length > 0) {
            for (File file : lib) {
                ...
                try (BufferedReader br = IOUtil.getReader(new FileInputStream(file), "UTF-8")) {
                    ...
                    LOG.warn("Init user library error :{}, path: {}" + e.getMessage() + file.getPath());
                }
            }

            LOG.info("Init user library ok!");


        } else {
            LOG.warn("Init user library  error :{} because : not find that file !" + path);
        }
