...
import com.tr.alg.util.DicReader;
...
public class NatureLibrary {
     ...
     * 初始化对照表
     */
    public static void init(String dicmappath, String dictablepath, String typestr) {
        String split = "\t";
        int maxLength = 0;
        String temp;
        String[] strs;
        // 加载词对照性表
        try {
            BufferedReader reader = DicReader.getReader(dicmappath, typestr);
            int p0;
            int p1;
            int p2;
        ...
        // 加载词性关系
        try {
            BufferedReader reader = DicReader.getReader(dictablepath, typestr);
